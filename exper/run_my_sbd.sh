...
if [ ${RUN_TRAIN} -eq 1 ]; then
    ...
    for pname in mytrain mysolver; do
                sed "$(eval echo $(cat sub.sed))" \
                        ${CONFIG_DIR}/${pname}.prototxt > ${CONFIG_DIR}/${pname}_${TRAIN_SET}.prototxt
    done
        CMD="${CAFFE_BIN} train \
           --solver=${CONFIG_DIR}/mysolver_${TRAIN_SET}.prototxt" 
...
if [ ${RUN_TEST} -eq 1 ]; then
    ...
    for TEST_SET in val; do
        ...
        MODEL=${EXP}/model/${NET_ID}/mytest${FOLD_SUFFIX}.caffemodel
        if [ ! -f ${MODEL} ]; then
                MODEL=`ls -t ${EXP}/model/${NET_ID}/mytrain${FOLD_SUFFIX}_iter_*.caffemodel | head -n 1`
        fi
        #
...
#       mkdir -p ${FEATURE_DIR}/${TEST_SET}/seg_score
        sed "$(eval echo $(cat sub.sed))" \
                ${CONFIG_DIR}/mytest.prototxt > ${CONFIG_DIR}/mytest_${TEST_SET}.prototxt
        CMD="${CAFFE_BIN} test \
             --model=${CONFIG_DIR}/mytest_${TEST_SET}.prototxt \
             --weights=${MODEL} \
...
#    fi
    ...
    for pname in mytrain mysolver2; do
                sed "$(eval echo $(cat sub.sed))" \
                        ${CONFIG_DIR}/${pname}.prototxt > ${CONFIG_DIR}/${pname}_${TRAIN_SET}.prototxt
    done
    CMD="${CAFFE_BIN} train \
         --solver=${CONFIG_DIR}/mysolver2_${TRAIN_SET}.prototxt \
         --weights=${MODEL}"
    if [ ${USE_GPU} -ne 0 ]; then
        CMD="${CMD} --gpu=${DEV_ID}"
    fi
...
#    for TEST_SET in val; do
        TEST_SET=${TEST_SET}${FOLD_SUFFIX}
        TEST_ITER=`cat ${EXP}/list/${TEST_SET}.txt | wc -l | sed 's/^ *//'`
        MODEL=${EXP}/model/${NET_ID}/mytest2${FOLD_SUFFIX}.caffemodel
        if [ ! -f ${MODEL} ]; then
            MODEL=`ls -t ${EXP}/model/${NET_ID}/mytrain2${FOLD_SUFFIX}_iter_*.caffemodel | head -n 1`
